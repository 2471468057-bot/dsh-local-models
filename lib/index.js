...
function apply(ctx) {
  ...
  function scrub(s) {
    ...
    return s.replace(/<available_skills>[\s\S]*?<\/available_skills>/gi, "").replace(/<skill\b[^>]*>[\s\S]*?<\/skill>/gi, "").replace(/<tool_call\b[^>]*>[\s\S]*?<\/tool_call>/gi, "").replace(/<\|\w+\|>/g, "").replace(/name="[^"]*">/g, "").replace(/-call-[a-z-]+>/gi, "").replace(/<\/?[a-z][a-z0-9-]*(?:\s[^>]*)?>/gi, "").replace(/[\s\u3000]+/g, " ").trim();
  }
