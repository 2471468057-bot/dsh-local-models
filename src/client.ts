...
export function apply(ctx) {
    ...
    async function call(method, args) {
      var res = await api.localModels[method](args === undefined ? {} : args)
    ...
    ctx.effect(function() {
      return function() { try { if (styleEl.parentNode) styleEl.parentNode.removeChild(styleEl) } catch (_e) {} }
    ...
    function StatusPill(props) {
      ...
      React.useEffect(function() {
        var alive = true
        function refresh() {
          call('status').then(function(r) { if (alive) sRun(r) }).catch(function() {})
        }
        refresh()
        var stop = setInterval(refresh, 4000)
        return function() { alive = false; clearInterval(stop) }
      }, [])
    ...
    function LocalModelsPage(props) {
      ...
      function refreshRun() {
        call('status').then(function(r) { sRun(r) }).catch(function() {})
      }
      function fetchDrafts(modelPath) {
        if (!modelPath) return
        call('drafts', { model: modelPath }).then(function(r) {
      ...
      React.useEffect(function() {
        ...
        var stop = setInterval(refreshRun, 5000)
        return function() { alive = false; clearInterval(stop) }
      }, [])

      function persist(next) {
        sData(next)
        call('saveAll', { data: next }).catch(function(e) { console.error('save failed', e) })
      }
      ...
      function doScan(dirOverride, base) {
        ...
        sScanning(true)
        return call('scan', { dir: dir }).then(function(r) {
          sScanning(false)
      ...
      function relaunch(modelPath) {
        call('relaunch', { model: modelPath }).then(function(r) {
          refreshRun()
      ...
      function launchNow(modelPath) {
        ...
        sLaunching(target)
        return call('ensure', { model: target }).then(function(r) {
          sLaunching('')
      ...
      return React.createElement('div', { className: 'lm2' },
        React.createElement('div', { className: 'lm2-setup' },
          React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '6px' } },
            ...
            run.running
              ? React.createElement('button', { className: 'lm2-btnd', onClick: function() { call('stop').then(refreshRun).catch(function() {}) } }, '■ 卸载')
          ...
          React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: '6px' } },
            ...
            React.createElement('button', { className: 'lm2-btnd', disabled: !data.backend.trim(), title: '检测 GPU 与内存', onClick: function() { call('gpu', { backend: data.backend }).then(function(r) { sSt(r.ok ? { t: 'ok', m: 'GPU 共 ' + r.totalMB + 'MB / 空闲 ' + r.freeMB + 'MB · 内存空闲 ' + (r.ramFreeMB || 0) + 'MB' } : { t: 'err', m: r.error }) }).catch(function(e) { sSt({ t: 'err', m: String(e) }) }) } }, '🎮'),
          ),
          ...
          React.createElement(StatusPill, { call: call }),
        ),
    ...
    slots.inject('settings.section', function() {
      return slots.register(
        { name: 'settings.section', id: 'local-models', order: 11, label: '本地模型', inject: function() { return { call: call } } },
        LocalModelsPage,
      )
    })
    slots.inject('conversation.composer.dock', function() {
      return slots.register(
        { name: 'conversation.composer.dock', id: 'local-turbo-status', order: 90, label: '本地模型状态', inject: function() { return { call: call } } },
        StatusPill,
      )
    })
  }
